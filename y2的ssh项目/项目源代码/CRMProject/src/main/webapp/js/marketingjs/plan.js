...
layui.use(['table','layer','form',"upload"],function(){
		...
		$("#ref").click(function(){//刷新
			...
			$('#chc_linkman').val("");//根据文本id取值  客户名
	        $('#chc_due_to').val("");//根据文本id取值   客户概要
		...
		table.render({
			  elem:'#test'//表格id
			,url:data+'/marketing/marketingAction_list.action?type=2'//所对应调用的接口
			,method:'post'		//提交方式
		    ,cols:[[
		    	/*根据数据库的表格所对应的名称  */
		    	 {field:'chc_id',height:80, width:100, title: '编号', sort: true,align:'center'}
		         ...
		         ,{field:'chc_due_to',height:80, width:160,title: '客户经理', align:'center'}
				  ...
				  ,{field:'right',align:'center',height:50, width:400, title: '操   作',templet: function(d){
					       		if(d.chc_status == 1){
					       		 ...
					       		 if(d.chc_status == 3){//开发成功
					                    return '<a style="color:#01AAED"  class="layui-btn layui-btn-xs layui-btn-radius layui-btn-primary" lay-event="exploit"><i class="layui-icon" ></i>机会详情</a>';
					             } 
        ...
        //查询 	
	  		reload: function () {
                ...
                var chc_due_to = $('#chc_due_to');//根据文本id取值   被指派人
                // 执行重载
	    ...
	    $('#hh').on('click', function () {//制定计划弹出框
	  	...
	  	$('.layui-form .layui-btn').on('click', function () {
  		  ...
  		  $('#h1').on('click', function () {//制定计划详情
  	        table.render({
    			 elem:'#sal'//表格id
         			,url:data+'/marketing/salPlanAction_PlanList.action?chc_id='+$("#chc_id3").val()//所对应调用的接口
         			,method:'post'
         			,page: true //提交方式
         		    ,cols:[[
         		    	/*根据数据库的表格所对应的名称  */
  		 ...
  		 $('#kfxq').on('click', function () {//开发成功/失败
 			  
   	        table.render({
     			 elem:'#sal'//表格id
          			,url:data+'/marketing/salPlanAction_PlanList.action?chc_id='+document.getElementById("chc_id4").value+''//所对应调用的接口
          			,method:'post'
          			,page: true //提交方式
          		    ,cols:[[
          		    	/*根据数据库的表格所对应的名称  */
...
//		table表格 监听 行内操作(编辑  以及  删除 按钮操作)  
	    table.on('tool(test)', function(obj){//大表格
	        ...
	        var layEvent = obj.event; //获得 lay-event 对应的值（也可以是表头的 event 参数对应的值）
	         ...
	         else if(layEvent === 'exploit'){ //开发成功/失败
	             layer.open({
	                 type: 1 //Page层类型
	                 ,skin: 'layui-layer-molv'
	                 ,area: ['70%', '70%']
	                 ...
	                 ,success:function(layero,index){//回显
		                 ...
		                 $('#chc_id4').val(data.chc_id);//主键id
		                 $('#chc_source4').val(data.chc_source);//机会来源
		                  $('#chc_cust_name4').val(data.chc_cust_name); //客户名称
		                 $('#chc_title4').val(data.chc_title);//概要
		                 $('#chc_rate4').val(data.chc_rate);//成功几率
		                 $('#chc_linkman4').val(data.chc_linkman);//联系人
		                 $('#chc_tel4').val(data.chc_tel);//联系电话
		                 $('#chc_desc4').val(data.chc_desc);//机会描述
		                 $('#user_id4').val(data.user_id);//创建人id
		                 $('#chc_create_by4').val(data.chc_create_by);//创建人名
		                 $('#chc_create_date4').val(data.chc_create_date);//创建人名字
		                 $('#chc_due_id4').val(data.chc_due_to);//指派给某名字
		                 $('#chc_due_date4').val(data.chc_due_date);//指派时的当前时间
		                 $('#chc_status4').val(data.chc_status);//状态
	                 },yes:function(index,layero){
	        	 ...
	        	 layer.confirm('确定机会开发成功？',{title:'确定'}, function(index){
	                 ...
	                 $.getJSON(urlex+'/marketing/salPlanAction_editState.action',{
	                	 ...
	                	 chc_status:3,
	                    	 ...
	                    	 if(ret>0){
	                    		 $.getJSON(urlex+'/client/clientAction_add.action',{
	                    			 
	                    			 cust_name:data.chc_cust_name,
	                    			 user_id: data.chc_due_id,//机会id    id
	                    			 cust_manager_name:data.chc_due_to,
	                    			 cust_tel:data.chc_tel,
	        	                     }, function(ret){
	        	                 });
	                    	 }
